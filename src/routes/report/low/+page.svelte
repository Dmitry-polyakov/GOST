<script>
	...
	// Моковые данные для высокого соответствия
	const check = {
		id: 1,
		file_name: 'test_вкр_1',
		file_size: 2048000,
		file_type: 'docx',
		...
		result: {
			compliance_score: 90,
			sections: {
				title_page: { is_compliant: true, errors: [] },
				contents: { is_compliant: true, errors: [] },
				main_content: { is_compliant: true, errors: [] },
				references: { is_compliant: false, errors: ['Неправильный отступ в списке литературы'] }
			},
			details: { total_pages: 20, formatting_errors: 1, citation_errors: 0, structure_errors: 0 }
		}
	};
